...
An authoritative `done` current state under a current `paused:` line is a finished wait still monitoring for merge or close, so the watcher admits it to that same cadence even while the agent process remains live; `done` with no pause declaration still wedge-escalates.
